    ...
    return (
        <div className="w-64 h-[calc(100vh-61px)] bg-white border-r border-gray-200/50 p-5 sticky top-[61px] z-20">
            <div className="flex flex-col items center justify-center gap-3 mt-3 mb-7">
                {user?.imageUrl ? (
                    <img src={user.imageUrl || ""} alt="Image" className='w-20 h-20 bg-slate-400 rounded-full'/>
                ) : (
            ...
            {SIDE_MENU_DATA.map((item, index) => (
                <button key={`menu_${index}`}
                        ...
                        ${activeMenu === item.label ? "bg-purple-500 text-white font-medium shadow-md hover:bg-purple-600":
                                "hover:bg-gray-100"} `}
                >
                    <itme.icon className="text-xl"/>
                    {item.label}
                </button>
            ))}
        </div>
